...
while read -r line ; do
  case $line in
    SYS*)
      ...
      # cpu
      if [ $(echo ${sys_arr[4]} | cut -d'/' -f1 | sed 's/%//') -gt ${cpu_alert} ]; then
      ...
      # gpu temp
      #gpu="%{F${color_sec_b2}}${sep_left}%{F${color_icon} B${color_sec_b1}} %{T2}GPU %{F- T1}${sys_arr[5]}"
      # mem
      ...
      # disk home
      diskh="%{F${color_icon}}${sep_l_left} %{T2}${icon_home}%{F- T1} ${sys_arr[8]}%"
      # wlan
    ...
    VOL*)
      ...
      ;;
    LIG*)
      light="%{F${color_sec_b2}}${sep_left}%{F${color_icon} B${color_sec_b1}} %{T2}${icon_light}%{F-T1} ${line#???}%"
      ;;
    GMA*)
      # Gmail
      gmail="${line#???}"
  ...
  # And finally, output
  printf "%s\n" "%{l}${wsp}${title}%{r}${mpd}${stab}${irc}${stab}${gmail}${stab}${cpu}${stab}${mem}${stab}${diskr}${stab}${diskh}${stab}${wland}${stab}${wlanu}${stab}${ethd}${stab}${ethu}${stab}${vol}${stab}${light}${stab}${date}${stab}${time}"
  #printf "%s\n" "%{l}${wsp}${title}"
done
